...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNightcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNightcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNightcoinAmount = strAnonymizeNightcoinAmount.remove(strAnonymizeNightcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNightcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNightcoinAmount*COIN) nMaxToAnonymize = nAnonymizeNightcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNightcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNightcoinAmount));
        strAnonymizeNightcoinAmount = strAnonymizeNightcoinAmount.remove(strAnonymizeNightcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNightcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeNightcoinAmount == 0){
