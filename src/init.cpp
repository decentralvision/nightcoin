...
void Shutdown()
{
    ...
    RenameThread("nightcoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: nightcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: nightcoin.pid)") + "\n";
    ...
    strUsage += "  -rpcthreads=<n>        " + _("Set the number of threads to service RPC calls (default: 12)") + "\n";
    strUsage += "  -rpckeepalive=<n>      " + _("RPC support for HTTP persistent connections (default: false)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizenightcoinamount=<n> " + _("Keep N Nightcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Nightcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nightcoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Nightcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Nightcoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nightcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Nightcoin to complete") << "\n";
    ...
    nAnonymizeNightcoinAmount = GetArg("-anonymizenightcoinamount", 0);
    if(nAnonymizeNightcoinAmount > 999999) nAnonymizeNightcoinAmount = 999999;
    if(nAnonymizeNightcoinAmount < 2) nAnonymizeNightcoinAmount = 2;
    ...
    LogPrintf("Anonymize Nightcoin Amount %d\n", nAnonymizeNightcoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1QUAN+1000 == (.1QUAN+100)*10
       10QUAN+10000 == (1QUAN+1000)*10
    */
